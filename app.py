...
with st.sidebar:
    ...
    st.download_button(
        ...
        data=csv_data,
        file_name=f"capacity_load_report_{pd.Timestamp.now().strftime('%Y%m%d')}.csv",
        mime="text/csv",
        use_container_width=True,
        help="Export dataset filtered by active date range, granularity, and rolling averages.",
    )
...
col5.metric("Discharge Ratio", "N/A" if pd.isna(ratio) else f"{ratio:,.2f}")
